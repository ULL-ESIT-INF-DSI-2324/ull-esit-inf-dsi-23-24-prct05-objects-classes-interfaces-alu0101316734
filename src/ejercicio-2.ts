...
export class Menu_instance implements menu_interface {
  constructor(
    ...
    public readonly no_heath: number,
  ) {}
  ordenar_platos(algorithm: heurisitica_interface): dishes {
    const sol = new Solver(this.platos, algorithm);
    this.platos = sol.logic();
    this.platos = this.platos.filter((dish) => {
      if (dish[1] > this.no_heath) return false;
      else return true;
    });
    return this.platos;
  }
...
export class Solver implements menu_solucion_interface {
  constructor(
    public readonly data: dishes,
    public tipo_heuristica: heurisitica_interface,
  ) {}
  logic(): dishes {
